...
- **Costo/beneficio:** lo stato XML verboso si moltiplica tra i turni e fa esplodere la bolletta di input; preferire Markdown/compatto per la prosa e JSON solo al confine dei tool. Verificare il caso Claude+XML sui dati (D7) prima di standardizzare su un unico formato.
- **Trappole:**
...
#### D7 — Il caso particolare di Claude: con i dati annidati, XML è davvero penalizzante anche su Claude, o no?
- **Il contesto del problema:** sulla maggior parte dei modelli testati in letteratura, XML è il **peggior** formato per far leggere dati annidati profondi — sia in accuratezza sia in costo (~+80% di token vs Markdown). Ma Claude è addestrato con moltissimo XML (è il formato nativo del suo tool-use), e su altri compiti regge l'XML meglio degli altri modelli.
- **Cosa testa [Q-IN]:** gli stessi dati annidati della Suite D5, dati a Claude Opus 4.8 (e per confronto Sonnet 4.6, Haiku 4.5) in YAML, Markdown, JSON e XML; si misura l'accuratezza delle risposte.
- **Ipotesi specifica:** su Claude, a differenza del pattern generale, XML **non** sarà il peggiore — sarà alla pari di YAML e Markdown, e sopra JSON.
- **Perché conta:** se costruisci su Claude e segui la raccomandazione XML di Anthropic per strutturare i prompt, devi sapere se quella scelta ti costa accuratezza quando dentro i prompt ci sono dati annidati. Se XML penalizza anche Claude, la regola diventa "XML per partizionare il prompt, ma YAML/Markdown per i dati dentro"; se Claude regge l'XML sui dati, puoi restare coerente con un solo formato. È l'unica domanda del set che nessuno studio pubblicato ha verificato su Claude — la più originale e la più difendibile in un articolo.
...
| D5 formato dati nel prompt | B | Dati annidati | Q-IN | ImprovingAgents `[PROD]` |
| D6 editing codice | B | Find-replace | Q-OUT | Checksum `[PROD]` |
| D7tab tabelle ampie (E7) | B | Tabellare (nuova) | Q-IN | nessuna (buco) |
| D7 XML su Claude / dati annidati | C | Dati annidati | Q-IN | nessuna su Claude (buco) |
